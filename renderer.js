...
let noise = FastSimplexNoise()
var SETTINGS = {
    gltfModelURL: 'FaceFilter/CustomMask/mask2.gltf',
    cubeMapURL: 'FaceFilter/Bridge2/',
...
//some globalz :
var THREEVIDEOTEXTURE, THREERENDERER, THREEFACEOBJ3D, THREEFACEOBJ3DPIVOTED, THREESCENE, THREECAMERA, CANVASELEMENT, faceGroup;
...
function init_threeScene(spec) {
    ...
    var sprite = textureLoader.load('textures/heart.png');
    var sprite1 = textureLoader.load('textures/rainbow.png');
    var sprite2 = textureLoader.load('textures/star.png');
    ...
    let parameters = [
        [
            [1.0, 0.2, 0.5], sprite1, 12
        ],
        [
            [0.95, 0.1, 0.5], sprite2, 0.5
        ],
        [
            [0.90, 0.05, 0.5], sprite, 0.5
        ]
...
function main() {
    JEEFACEFILTERAPI.init({
        ...
        NNCpath: 'FaceFilter/', //root of NNC.json file
        ...
        callbackTrack: function (detectState) {
            ...
            if (ISDETECTED) {
                ...
                // ADD TO BUFFER
                if (rotationBuffer.length < BUFFER_SIZE) {
                    rotationBuffer.push(detectState.rz);

                } else {
                    rotationBuffer.shift();
                    rotationBuffer.push(detectState.rz);
                    calculateShake(rotationBuffer);
                }
                ...
                for (var i = 0; i < THREESCENE.children.length; i++) {
                    var object = THREESCENE.children[i];
...
function calculateShake(buffer) {
